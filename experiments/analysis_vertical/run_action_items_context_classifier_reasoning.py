"""Run V4 action-item context classifier with Ollama reasoning enabled.

This runner uses only local artifacts and stores reasoning traces separately.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run V4 classifier with reasoning enabled.")
    ...
    experiment_id = f"action_items-v4-context-reasoning-{args.recording_id}-{timestamp}"
    ...
    write_json(
        run_dir / "manifest.json",
        {
            ...
            "target": "action_items",
            "vertical_variant": "v4_context_classifier_reasoning",
    ...
    model = ChatOllama(model="gemma4:e2b", format="json", temperature=0, reasoning=True)
    slice_summaries = []
    promoted = []
    ...
    for source_slice in candidate_summary["slices"]:
            ...
            [
                SystemMessage(
                    content=(
                        ...
                        "Think carefully, then return only valid JSON in the final answer. "
                        "Obey the hard negative rules and classify every candidate_id exactly once."
                    )
                ),
        ...
        (slice_dir / "reasoning.txt").write_text(
